...
interface PostJobProps {
  onJobPosted: () => Promise<void>; // Callback to refresh job listings
  onClose: () => void;              // Function to close the modal
}

const PostJob: React.FC<PostJobProps> = ({ onJobPosted, onClose }) => {
  const [jobTitle, setJobTitle] = useState('');
  const [jobDescription, setJobDescription] = useState('');
  const [jobAddressLocation, setJobAddressLocation] = useState('');
  const [jobSalary, setJobSalary] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await fetch('/api/post-job', {
      ...
      body: JSON.stringify({
        ...
        companyName,
        userId,
        candidateResponseCount: 0, // Default value
        totalApplicantsCount: 0,   // Default value
      }),
    });
    ...
    if (response.ok) {
      const { url } = await response.json();
      setJobUrl(url);
      await onJobPosted(); // Call the onJobPosted function to refresh the job list
      ...
      <div className="post-job-container">
        ...
        {jobUrl && (
          <div className="mt-4">
            <p className="text-green-600">Job posted successfully!</p>
            <p>
              View job: <a href={jobUrl} className="text-blue-600">{jobUrl}</a>
            </p>
          </div>
        )}
      </div>
